...
def reconstruct_path(came_from_node, current, draw): # creates shortest path from start to end using Manhattan distance
...
def algorithm(draw, grid, start, end):
    ...
    came_from_node = {}
    gCost = {tile: float("inf") for row in grid for tile in row} # current shortest distance to get from the start node to node n
    gCost[start] = 0
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            if temp_gCost < gCost[neighbor]:
                ...
                fCost[neighbor] = temp_gCost + heuristic(neighbor.get_pos(), end.get_pos())
